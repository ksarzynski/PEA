...
int BranchAndBoundSolver::firstMin(int vertex){
    int min = INT_MAX;
    for (int i = 0; i < this->size; i++)
...
int BranchAndBoundSolver::secondMin(int vertex){
    int min1 = INT_MAX;
    int min2 = INT_MAX;
    for (int i = 0; i < this->size; i++){
        if (vertex == i)
...
int BranchAndBoundSolver::getStartingLowerBound(){
    int lowerBound = 0;
    for (int i = 0; i < this->size; i++)
...
bool BranchAndBoundSolver::isVisited(int index, const std::vector<int>& path) {
    for(int i : path)
        if(i == index)
            return true;
    return false;
